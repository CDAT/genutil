...
class ADDPAR():
    ...
    """
    def __init__(self):
        from pymars import FLOAT_DTYPE, INT_DTYPE
        self.maxdph = 2000
        self.mpr = 10
        self.mtr = 5
        self.beta = 1.0
    ...
    def addpar(self,bfIndex):
        ...
        #pymars.debug.info('itr='+repr((self.itr,self.itr - self.que[2,J])))
        #pymars.debug.info('temp='+repr(temp[J]))
        self.m[0:self.last+1] = temp.argsort()
        self.kp = max(0, self.last-self.mpr)
        #pymars.debug.info('kp, mpr='+repr((self.kp, self.mpr)))
        return
