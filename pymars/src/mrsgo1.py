...
import pymars, time, sys, scipy, numpy, operator, pdb
from pymars.mrsgo2 import findBestKnot
#from mrsgo2Cython import findBestKnot
from pymars.elg import elg
from pymars.mnspan import mnspan
from pymars.jft import jft
from pymars.jf import jf
from pymars.csp import csp
from pymars.finalDecision import *
from pymars.border import *
from pymars import LOG, logger, debug, parameters, ARRAY_SIZE, FLOAT_DTYPE, INT_DTYPE, MPI
...
def findBestChildVariable(parent, nextVar, 
    ...
    END = time.time()
    if 'marsgo1' in pymars.TIME.keys(): 
        t0 = END-START
        pymars.TIME['marsgo1'][0] += t0
        pymars.TIME['marsgo1'][2] += 1
        if len(VARIABLES) > 1:
            pymars.TIME['marsgo1'][1] += t0
            pymars.TIME['marsgo1'][3] += 1
...
def computeProposedHockeyStick((variable, X, MM, parent, 
    ...
    if X[MM[1]] < X[MM[n]]:
        ...
        if jf(parent, variable, tb) == 0: 
            ...
            if ja >= 0: 
                if elg(pymars.NEST0, pymars.SETINT0, variable, parent, lx, tb, cm):
                    if ja == 0:
                        if lbf != 0:
                            nnt, partialEval = blf0(pymars.NEST0, parent, 0, n, X, w, cm, parentEval)
                            lbf = 0
                            mn, me, mel = mnspan(ms, alf, nep, nnt)
                    else: 
                        nnt, partialEval = blf0(pymars.NEST0, parent, ja, n, X, w, cm, parentEval)
                        lbf = 1
                        if nnt > nmin: 
                            mn, me, mel = mnspan(ms, alf, nep, nnt)
