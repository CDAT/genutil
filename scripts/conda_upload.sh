...
conda build -c conda-forge -c uvcdat/label/nightly -c uvcdat ${PKG_NAME} --numpy=1.12
anaconda -t $CONDA_UPLOAD_TOKEN upload -u $USER -l nightly $CONDA_BLD_PATH/$OS/$PKG_NAME-`date +%Y.%m.%d`-np112py27_0.tar.bz2 --force
