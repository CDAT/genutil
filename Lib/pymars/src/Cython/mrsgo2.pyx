...
def findBestKnot(int n, 
                 ...
                 np.ndarray[np.int32_t, ndim=1] lx, 
                 np.ndarray[np.float64_t, ndim=1] cm, 
                 ...
                 #n, X, y, w, lx, cm, MM,  partialEval, db, 
    ...
    """
    #START = time.time()
    cdef double knot, coef, START, END
    cdef int k1
    cdef dict coefAndKnot
...
def getPotentialKnots(int n, 
                      ...
                      int mn,
                      int nnt): 
                      #n, X, MM, w, partialEval, me, mel, mn, nnt
    ...
    cdef list potentialKnots
    cdef int nnl, nst, nnr, j, j0, mj
    cdef double h
...
def bestKnot(int n, 
             ...
             int kr,
             np.ndarray[np.float64_t, ndim=1] DY):
             #n, X, y, yb, w, sw, MM, partialEval, db, 
    ...
    cdef list inputs, outputs, potentialKnots
    cdef double trialKnot, coef, Coef, knot, v, u, t, we, se, sy, dy, txt, xt
    cdef double V, T, U, WE, SE, SY, DYY
    cdef np.ndarray[np.float64_t, ndim=1] D, DB, ST, SU
    cdef bint PARALLEL, FIRST_TRIAL, LastPair, newCoef
    cdef int MAXINPUTS, I, j, j0
...
def computeCoef(double trialKnot, 
    ...
    cdef np.ndarray[np.float64_t, ndim=1] ST
    cdef np.ndarray[np.float64_t, ndim=1] SU    
...
def checkCoef(double coef, 
              ...
              np.ndarray[np.float64_t, ndim=1] DY):
              #coef, eps, kr, sw, v, we, se, dy, D, DY):
