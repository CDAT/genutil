...
from .blf import blf
from .blf0 import blf0
from .jf import jf
from .elg import elg
from .mnspan import mnspan
from .sscp import sscp
from .lsf1 import lsf1
from .coefpr import coefpr
from .update import update
from .bkstp import bkstp
from .mrsgo1 import findBestChildVariable
from .array import array
from .phi import phi
from .holl import holl
...
#    debug.info(' ')
    ...
    for ll in range(2, nli+1):
        b, A, a, k = \
            bkstp(D, nBF, XB, yb, parameters['marsgo']['alr'], DP)
        if k != 0:
            if parameters['cvmars']['ix'] != 0:
                i, j = array(kl+3, n)
                cvStuff[i,j] = b/sw
...
def buildNextBasisFunction(parent, nextVar,
                           n, p, x, y, yb, w, sw, lx, tb, cm, mm, mi, 
                           db, bfIndex, bfIndexLast, evaluations, 
                           ms, fv, newHS, tx1, txi, nBasisFunctions, nep, 
                           ssq, rsq, asq0,kcp0, jas, kr, nc, kcp, k1, DY, 
            ...
            if nnt <= nmin:
                ...
                if nep == 0:
                    ...
                    if nnt <= max(me,mel):
                        ...
                        #print 'rsq, ssq, kr, k1 before=', rsq, ssq, kr, k1
                        #print 'before findBestChildVariable'
                        #(nextVar, newHS, cm, db, fv, tx1, txi, rsq, ssq, kr, k1,  
                        # kcp0, kcp, ja, jas,  nnt, nc,  DY) = \
                        #(nextVar, newHS, cm,  tx1, txi,  kcp0, kcp, ja, jas) = \
                        (nextVar, newHS, cm,  tx1, txi, jas ) =\
                        ...
                        #print 'rsq, ssq, kr, k1 after=', rsq, ssq, kr, k1
                        #print ' '
                        #print 'after findBestChildVariable'
        parent, nextVar = genutil.pymars.ADDPAR0.nxtpar(parent, nextVar)
    #print 'end build'
...
def printBasisFunction(xm, xs, bfIndex, mtot, kr, tcst, cm, newBF, rsq, sw, wn):
    ...
    if cm[2*variable] > 0.0:
        ...
        #debug.info('xk, xm, xs, tx, bfIndex'+repr((xk, xm[variable], xs[variable],newBF[3], bfIndex)))
        if bfIndex == mtot+1: 
