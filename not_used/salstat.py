...
def differencesquared(x, y, axis=0):
    ...
    if isvar:
        ...
        if 'units' in xatt.keys():
            diff.units = xatt['units'] + '*' + xatt['units']
...
def _betai(a, b, x, ITMAX=200, EPS=3.0E-7):
    ...
    ans = numpy.ma.where(numpy.ma.logical_and(numpy.ma.logical_not(c2), c1),
                         1.0 - bt * _betacf(b, a, 1.0 - x, ITMAX, EPS) / b, ans)
    return ans
...
def sumsquares(x, axis=0):
    ...
    if not ax is None:
        ...
        if 'units' in xatt.keys():
            sq.units = xatt['units'] + '*' + xatt['units']
    # numpys only ?
    if not numpy.ma.isMA(x):
        sq = sq.filled(1.e20)
    return sq
...
def unbiasedvariance(x, axis=0):
    ...
    if not ax is None:
        ...
        if 'units' in xatt.keys():
            out.units = xatt['units'] + '*' + xatt['units']
    # numpys only ?
    if not numpy.ma.isMA(x):
        out = out.filled(1.e20)
    return out
