...
c      close(99)
  ...
  100 format(/,' MARS run time = ',f13.6, ' seconds')
    ...
    3 if((i).gt.(im(1))) go to 4
      fmn(i)=fm(i)
      go to 2
    4 call slice1(mi,flg,xs,im(3),im(4),x,im(5),fm(im(11)),fm(im(12)),
     1fm(im
...
c  14 continue
   ...
   18 return
     ...
     1,dp,mm)
      implicit none
c this declaration of mmwork is only a dummy to satify the compiler
      integer mmwork(10)
...
c     write(it,13) s,t
    6 if(it .le. 0) go to 7
      if(il.eq.0) call anova (p, n,x,y,w,nk,it,tb,cm,lp,lv,sp,dp)
      if(il.gt.0) call anoval(n,x,y,w,nk,il,it,az,tb,cm,lp,lv,sp,dp)
    7 call ccoll (n,p,mi, nk,tb,cm,kp,kv,lp,lv,mm)
      call cubic(mi, n,p,x,y,w,nk,it,tb,cm,kp,kv,lp,lv,bz,tc,sp,sp(i1),
     1sp(i1+2*p),mm,dp)
      if(il .le. 0) go to 9
...
c     write(it,14) s,t
    9 if(it.gt.0) call varimp (n,p,x,y,w,nk,il,it,az,tb,cm,sp,sp(p+1),dp
     1)
      call orgpl(p,sp(im),sp(is),nk,tb,cm)
      call orgpc(nk,p, sp(im),sp(is),lp,lv,tc)
...
c      write(it,33) j,(js(i+2*nv),i=1,na)
   ...
   37 format(' function not printed (more than',i3,' categorical values)
     1.')
      ...
      subroutine slice1 (mi, flg,xs,n,p,x,nk,az,tb,cm,kp,kv,lp,lv,bz,tc,
     1azn,tbn,kpn,kvn,  lpn,lvn,bzn,tcn,sp,mm)
      implicit none
      integer n,p,mi,nk,kp(5,*),kv(2,*),lp(3,*),lv(*),kpn(5,*),kvn(2,*),
     1lpn(3,*),lvn(*),mm(*)
...
c      if(it.gt.0) write(it,'('' sliced mars model = constant.'')')
   ...
   10 if(it.gt.0) call slova(nk,it,tbn,ni,lpn,lvn)
      call ccoll(n,p,mi, nk,tbn,cm,kpn,kvn,lpn,lvn,mm)
...
c add p as an argument for the python interface
      subroutine orgpl (p, xm,xs,nk,tb,cm)
      implicit none
      integer nk
...
c add p as an argument for the python interface
      subroutine anova (p,n,x,y,w,nk,it,tb,cm,lp,lv,t,d)
      implicit none
      integer jjj,p,n,nk,it,lp(3,*),lv(*)
...
c add mi as an argument for the python interface
      subroutine cubic (mi, n,p,x,y,w,nk,it,tb,cm,kp,kv,lp,lv,bz,tc,t,z,
     1sc,js,d)
      implicit none
      integer mi, n,p,nk,it,kp(5,*),kv(2,*),lp(3,*),lv(*),js(*)
...
c add nk,p as an argument for the python interface
      subroutine orgpc (nk,p, xm,xs,lp,lv,tc)
      implicit none
      integer lp(3,*),lv(*)
      double precision xm(*),xs(*),tc(*)
      integer nk,p, la,l1,l,nt,lb,j
      la=1
      l1=la
...
c add n,p,mi as an argument for the python interface
      subroutine ccoll (n,p,mi, nk,tb,cm,kp,kv,lp,lv,jv)
      implicit none
      integer n,p,mi
